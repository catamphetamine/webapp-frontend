	...
	switch (part.type) {
		...
		case 'link':
			if (part.autogenerated) {
				if (options.messages && options.messages.post && options.messages.post.linkTo) {
					return `(${options.messages.post.linkTo.replace('{0}', getDomainName(part.url))})`
				}
		...
		case 'code':
			if (!part.inline && options.excludeCodeBlocks) {
				if (options.messages && options.messages.contentType && options.messages.contentType.code) {
					return '(' + options.messages.contentType.code.toLowerCase() + ')'
				}
				return
			}
			return getContent()
