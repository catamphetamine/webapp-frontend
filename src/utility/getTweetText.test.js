...
function getTweetTextTest(html, text) {
	expectToEqual(getTweetText(html, { messages: { link: 'link' } }), text)
}
...
describe('getTweetText', () => {
	it('should get tweet text', () => {
		getTweetTextTest(
			...
			'Sunsets don\'t get much better than this one over @GrandTetonNPS. #nature #sunset (link)'
		)
	})
	...
	it('should filter-out tags in the beginning and in the end, but not in the middle', () => {
		getTweetTextTest(
			...
			'#start We, the undersigned 26 international human rights, media and Internet freedom organisations call on #Russia to stop blocking @Telegram and cease its relentless attacks on Internet freedom more broadly. #end (link) (link)'
		)
	})
})