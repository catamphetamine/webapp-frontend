...
export default function PostInlineQuote({ url, autogenerated, kind, kinds, children }) {
	const className = classNames('post__inline-quote', 'post__quote-block', {
		'post__inline-quote--link': url
	})
	const quote = (
		<Quote autogenerated={autogenerated} kind={kind} kinds={kinds}>
			{children}
		</Quote>
	)
	if (url) {
...
function Quote({ children, kinds, ...rest }) {
	if (Array.isArray(children)) {
		return children.map((quote, i) => (
			<React.Fragment key={i}>
				{i > 0 ? <br/> : null}
				<QuoteBlock kind={kinds && kinds[i]} {...rest}>
					{quote}
				</QuoteBlock>
			</React.Fragment>
		))
